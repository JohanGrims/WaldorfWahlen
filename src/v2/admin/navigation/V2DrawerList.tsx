...
import V2VoteDrawer from "./V2VoteDrawer";
import routes from "../../../admin/navigation/routes.json";
...
export default function V2DrawerList({
  onClose = () => {},
  mobile,
}: DrawerListProps) {
  ...
  React.useEffect(() => {
    ...
    setExpiredVotes([]);
    setScheduledVotes([]);
    getDocs(collection(db, "schools/SCHOOLID/v2_votes"))
      .then((data) => {
        data.docs.map((e) => {
  ...
  if (!pages.includes(active)) {
    return <V2VoteDrawer onClose={onClose} />;
  }
      ...
      >
        ...
        >
          <DrawerItem
            ...
            title={"Erstellen"}
            icon={"create"}
            onClick={() => navigateTo("/v2/admin/new")}
          />
        </mdui-tooltip>
                    ...
                    .map((e) => (
                      ...
                      >
                        <DrawerItem
                          active={active === e.id}
                          title={e.title}
                          onClick={() => navigateTo(`/v2/admin/vote/${e.id}/manage`)}
                        />
                      </mdui-tooltip>
                    ))}
                </>
