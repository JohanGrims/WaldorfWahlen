...
export default function V2Results() {
  const { vote, options, results, choices: rawChoices } = useLoaderData() as LoaderData;

  const { studentMap } = useDecryption();

  const choices = React.useMemo(() => {
    return rawChoices.map(c => ({
      ...c,
      name: studentMap[c.id]?.name || `Unbekannt (${c.id.substring(0,6)})`,
      grade: studentMap[c.id]?.grade || 0,
      listIndex: studentMap[c.id]?.listIndex || 0,
    }));
  }, [rawChoices, studentMap]);
  ...
  function publishResults() {
    confirm({
      ...
      onConfirm: () => {
        setDoc(
          doc(db, `schools/SCHOOLID/v2_votes/${vote.id}`),
          {
            result: true,
          },
          { merge: true }
  ...
  function addComment(id: string) {
    prompt({
      ...
      onConfirm: (comment: string) => {
        ...
        setDoc(
          doc(db, `schools/SCHOOLID/v2_votes/${vote.id}/results/${id}`),
          {
            comments: [
              ...existingComments,
              {
...
V2Results.loader = async function loader({ params }: LoaderFunctionArgs) {
  const { id } = params as { id: string };
  const vote = await getDoc(doc(db, `schools/SCHOOLID/v2_votes/${id}`));
  const voteData = { id, ...vote.data() } as VoteData;
  const options = (
    await getDocs(collection(db, `schools/SCHOOLID/v2_votes/${id}/options`))
  ).docs.map((doc) => {
    return { id: doc.id, ...doc.data() };
  }) as OptionData[];

  const results = (
    await getDocs(collection(db, `schools/SCHOOLID/v2_votes/${id}/results`))
  ).docs.map((doc) => {
    return { id: doc.id, ...doc.data() };
  }) as ResultData[];

  const choices = (
    await getDocs(collection(db, `schools/SCHOOLID/v2_votes/${id}/choices`))
  ).docs.map((doc) => {
    return { id: doc.id, ...doc.data() };
  }) as ChoiceData[];
