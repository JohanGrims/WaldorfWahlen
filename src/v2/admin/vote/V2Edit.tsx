...
export default function V2Edit() {
  const {
    vote,
    ...
    try {
      ...
      removedOptions.map((opt) =>
        confirm({
          ...
          onConfirm: async () => {
            await deleteDoc(
              doc(db, `schools/SCHOOLID/v2_votes/${vote.id}/options/${opt.id}`)
            );
          },
        })
      );
      const optionsPromises = options.map(async (e) => {
        return setDoc(
          doc(db, `schools/SCHOOLID/v2_votes/${vote.id}/options/${e.id}`),
          {
            title: e.title,
            max: e.max,
            teacher: e.teacher,
      ...
      navigate(`/v2/admin/vote/${vote.id}/manage`);
    } catch (error) {
      ...
      onConfirm: async () => {
        try {
          await deleteDoc(
            doc(
              db,
              `schools/SCHOOLID/v2_votes/${vote.id}/proposals/${proposal.id}`
            )
          );
...
V2Edit.loader = async function loader({ params }: LoaderFunctionArgs) {
  try {
    const { id } = params;
    ...
    const proposals = await getDocs(
      collection(db, `schools/SCHOOLID/v2_votes/${id}/proposals`)
    );
