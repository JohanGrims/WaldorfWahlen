...
export default function V2Assign() {
  const {
    vote,
    choices: rawChoices,
    options,
  ...
  function saveResults() {
    sortedResults.forEach(([key, value]) => {
      setDoc(
        doc(db, `schools/SCHOOLID/v2_votes/${vote.id}/results/${key}`),
        {
          result: value,
        },
        {
...
V2Assign.loader = async function loader({ params }: LoaderFunctionArgs) {
  ...
  const choices = await getDocs(
    collection(db, `schools/SCHOOLID/v2_votes/${id}/choices`)
  );
  ...
  const options = await getDocs(
    collection(db, `schools/SCHOOLID/v2_votes/${id}/options`)
  );
  ...
  const results = await getDocs(
    collection(db, `schools/SCHOOLID/v2_votes/${id}/results`)
  );
